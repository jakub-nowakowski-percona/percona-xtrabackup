...
# describing the backup. It does so over the same connection that is used
# to take the backup, so it needs that server to accept writes.
#
...
set +e
$XB_BIN $XB_ARGS --backup --history=$history_name \
...
if [ $rc -eq 0 ]
then
    die "--history on a super_read_only secondary was expected to fail"
fi
